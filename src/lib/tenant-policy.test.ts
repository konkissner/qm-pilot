...
describe.runIf(run)('ZenStack tenant isolation', () => {
  ...
  beforeAll(async () => {
    ...
    const userA = await prisma.user.create({ data: { tenantId: tenantAId, email: `a-${Date.now()}@example.com`, name: 'A U', firstName: 'A', lastName: 'U', roles: { connect: { id: tenantA.roles[0].id } } } });
    userAId = userA.id;
  });
  ...
  it('blocks cross-tenant write', async () => { const db = getEnhancedPrisma({ id: userAId, tenantId: tenantAId, status: 'active' }); await expect(db.user.create({ data: { tenantId: tenantBId, email: `x-${Date.now()}@example.com`, name: 'X Y', firstName: 'X', lastName: 'Y' } })).rejects.toThrow(); });
});
...
describe.runIf(run)('users service', () => {
  ...
  beforeAll(async () => {
    ...
    const gl = await prisma.user.create({ data: { tenantId, email: `gl-${Date.now()}@example.com`, name: 'GL U', firstName: 'GL', lastName: 'U', roles: { connect: { id: tenant.roles.find((r) => r.key === 'gl')!.id } } } });
    const qmb = await prisma.user.create({ data: { tenantId, email: `qmb-${Date.now()}@example.com`, name: 'QMB U', firstName: 'QMB', lastName: 'U', roles: { connect: { id: tenant.roles.find((r) => r.key === 'qmb')!.id } } } });
    const target = await prisma.user.create({ data: { tenantId, email: `t-${Date.now()}@example.com`, name: 'T U', firstName: 'T', lastName: 'U' } });
  ...
  it('reject rp by QMB', async () => { const t = await prisma.user.create({ data: { tenantId, email: `f-${Date.now()}@example.com`, name: 'F U', firstName: 'F', lastName: 'U' } }); await expect(assignRole(prisma, { id: qmbId, tenantId, roleKeys: ['qmb'] }, t.id, 'rp')).rejects.toBeInstanceOf(UserServiceForbiddenError); });
  it('allow rp by GL', async () => { const t = await prisma.user.create({ data: { tenantId, email: `g-${Date.now()}@example.com`, name: 'G U', firstName: 'G', lastName: 'U' } }); await assignRole(prisma, { id: glId, tenantId, roleKeys: ['gl'] }, t.id, 'rp'); const u = await prisma.user.findUniqueOrThrow({ where: { id: t.id }, include: { roles: true } }); expect(u.roles.some((r) => r.id === rpRoleId)).toBe(true); });
});
